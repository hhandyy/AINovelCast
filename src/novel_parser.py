...
【角色命名原则】
3. "role" 字段**只能是以下两类之一**：
   - 具体的角色名字（如“韩立”“南宫婉”“萧师姐”等），或
   -在当前片段中明确出现、且具有唯一指代性的关键代称（如 “萧师姐”“矮胖青年”“黑衣老者”等）
   上述名称或代称必须在原文当前或紧邻上下文中实际出现过，并满足：
       该代称在本片段中多次用于指代同一说话人，且无法推测得知其真实姓名；
       通过上下文可确认其身份稳定、不可替换（即不是泛指某类人，而是特指某个参与多轮对话的次要角色）；
   - 除上述情况外，所有未具名、身份模糊、群体性或仅单次出现的说话者（如 “一名修士”“远处传来声音”“众人齐声” 等），其话语及引导语一律由“旁白”作为"role" 转述。
4. 判断某句对话是否归属具体角色，需同时满足：
   a) 该角色的名字已在原文中出现（不一定在同一句）；
   b) 通过上下文、自称（如“本座”“小女子”）、对话对象称呼（如“韩兄”“银月姑娘”）或情节逻辑，能合理地确定说话人就是该具体角色。
   若满足，则 "role" 使用该具体名字，不要使用临时出现的称呼和代称。
5. 严禁因“原文未写‘XXX说’”而将可合理归因于已知名字角色的台词转为旁白；也严禁为无名字角色虚构或保留模糊称谓。

【条目拆分规则】
6. 必须严格按语义单元与原文结构拆分条目：
